...
module.exports = {
    ...
    createExchange(body, _cb) {
        if (!body || !body.vhost) {
            _cb(null, { errorMessage : 'Vhost not provided' });
            return;
        }
        if (!body.exchange) {
            _cb(null, { errorMessage : 'Exchange name not provided' })
        ...
        if (!body.type) {
            _cb(null, { errorMessage : 'Exchange type not provided' });
            return;
        }
        var putBody = {
            type : body.type,
    ...
    deleteExchange(body, _cb) {
        ...
        const path = `/api/exchanges/${encodeURIComponent(body.vhost)}/${encodeURIComponent(body.exchange)}`;
